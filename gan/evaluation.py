...
def compare_distributions(real: pd.DataFrame, synth: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    """
...
def train_classifier_test(real: pd.DataFrame, synth: pd.DataFrame, label_col: str = "Class") -> float:
    """
    ...
    X_train, _, y_train, _ = train_test_split(X_s, y_s, test_size=0.2, stratify=y_s, random_state=42)
