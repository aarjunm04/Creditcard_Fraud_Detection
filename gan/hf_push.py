...
def push_dataset_to_hub(
    ...
    if not token:
        raise RuntimeError(
            "No HuggingFace token found. Run `huggingface-cli login` first."
        )

    repo_url = api.create_repo(repo_id=repo_id, private=private, exist_ok=True)
    repo = Repository(
        local_dir=Path("hf_tmp"), clone_from=repo_url, use_auth_token=token
    )
...
def push_model_to_hub(
    model_path: str | Path,
    repo_id: str,
    filename: str = "ctgan_model.joblib",
    private: bool = True,
):
    """
